...
{/*
	//system("color f2");
		...
		FileWatchList* a = new HTMLWatchList{};
		/*
		...
		*/
	/*
		Controller ctrl{ repo, a, FilmValidator{} };
		UI ui{ ctrl };
		ui.run();
	...
	*/
	...
	try
	{
	printf("Give the type of the WatchList file: CSV or HTML\n");
	std::string type;
	cin >> type;
	if (type == "CSV")
	{
		...
		Repository repo("films.txt");
		FileWatchList* a = new CSVWatchList{};
		Controller ctrl(repo, a, FilmValidator{});
		UI ui(ctrl);
		ui.run();
		delete a;
