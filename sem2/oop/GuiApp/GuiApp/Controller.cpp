...
std::vector<Film> Controller::filterByDuration(const int duration)
{
	vector<Film> v = this->getRepo().getFilms();
	vector<Film> filter;
	bool ok = false;

	for (auto it : this->getRepo().getFilms())
	{
		if (it < duration)
		{
			filter.push_back(it);
			//cout << it.getTitle() << " - " << it.getGenre() << " - " << it.getYear() << "; " << it.getLikes() << ";" << it.getDuration() << "\n";
			ok = true;
		}
	}
	return filter;
