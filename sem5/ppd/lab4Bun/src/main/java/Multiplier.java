...
public class Multiplier implements Callable<Object> {
    ...
    public Object call() throws Exception {
        positions.forEach(position -> {
            m_result.put(Pair.with(position.getValue0(),position.getValue1()),IntStream.range(0, Main.size).map(i -> m1.get(Pair.with(position.getValue0(),i)) * m2.get(Pair.with(i,position.getValue1()))).sum());
        });
        return null;
    }
}
