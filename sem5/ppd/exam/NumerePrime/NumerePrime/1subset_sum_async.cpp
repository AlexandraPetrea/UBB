...
struct Solver
{
	...
	void search(std::vector<bool>& subset, size_t pos, int targetSum, unsigned nrThreads)
	{
		...
		if (nrThreads > 1) {
			...
			subset2[pos] = false;
			std::future<void> other = std::async([this, pos, &subset2, targetSum, nrThreads]() 
			{search(subset2, pos + 1, targetSum, nrThreads / 2); });
			subset[pos] = true; nrThreads - nrThreads / 2;
...
{
	long long nrThreads;
	if (argc != 2 || 1 != sscanf_s(argv[1], "%lld", &nrThreads)) {
		fprintf(stderr, "Usage: subset_sum_async nrThreads\n");
		return 1;
	}
