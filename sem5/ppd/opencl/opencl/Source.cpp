...
int main(int argc, const char * argv[]) {
	...
	// GPU in our system.                                             // 2
	clCreateCommandQueueWithProperti
	...
	if (queue == NULL) {
		queue = create_dispatch_queue(CL_DEVICE_TYPE_CPU, NULL);
	}
	...
	//cl_device_id gpu = clGetDeviceIDs;
	...
	for (i = 0; i < NUM_VALUES; i++) {
		test_a[i] = (cl_float)(rand() % MAXVALUE);
		test_b[i] = (cl_float)(rand() % MAXVALUE);
	}
	...
	void* mem_a = local_malloc(sizeof(cl_float) * NUM_VALUES, test_a,
		CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR);
	void* mem_b = gcl_malloc(sizeof(cl_float) * NUM_VALUES, test_b,
		CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR);

	// The output array is not initalized; we're going to fill it up when
	// we execute our kernel.                                             // 4
	void* mem_out =
		gcl_malloc(sizeof(cl_float) * (NUM_VALUES + NUM_VALUES - 1), NULL, CL_MEM_WRITE_ONLY);
	...
	dispatch_sync(queue, ^{
		...
		size_t wgs;
		gcl_get_kernel_block_workgroup_info(poly_mult_naive_kernel,
		...
		cl_ndrange range = {                                              // 6
			...
			{NUM_VALUES + NUM_VALUES, 0, 0},    // The global range—this is how many items
		...
		poly_mult_naive_kernel(&range, (cl_float*)mem_a, (cl_float*)mem_b, (cl_float*)mem_out, (cl_int)NUM_VALUES);
		...
		gcl_memcpy(test_out, mem_out, sizeof(cl_float) * (NUM_VALUES + NUM_VALUES - 1));
	...
	if (validate(test_a, test_b, test_out)) {
		fprintf(stdout, "The polynoms were properly multiplicated.\n");
	}
