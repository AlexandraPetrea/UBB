...
class Scanner:
    ...
    def detect(line: str) -> List[str]:
        tokens = [x for x in re.split('([^\w."])', line) if x != '' and x != ' ']
        return tokens
    ...
    def write_to_file(self):
        with open(self.output_file, "w") as f:
            ...
            f.write('\n'.join('{}: {}'.format(self.codification_table[x], x) for x in self.codification_table))
            ...
            f.write('\n'.join('{}: {}'.format(x[0], x[1]) for x in self.program_internal_form))
                    ...
                    if self.is_keyword(tokens[i]) or self.is_operator(tokens[i]) or self.is_separator(tokens[i]):
                        if 0 < i < len(tokens) and tokens[i] == "-" and self.is_operator(tokens[i - 1]):
                            if self.is_number_constant(tokens[i + 1]):
                                ...
                                position = self.get_position(self.constants_symbol_table, to_be_added)
                                self.program_internal_form.append((self.codification_table[self.CONSTANT], position))
                                i += 1
                                column += 2
                            else:
                                raise LexicalException(f"Unexpected token at {index}, {column}: {tokens[i]}")
                        else:
                            ...
                            self.program_internal_form.append((self.codification_table[to_be_added], (-1, -1)))
                    elif self.is_identifier(tokens[i]):
                        position = self.get_position(self.identifiers_symbol_table, tokens[i])
                        self.program_internal_form.append((self.codification_table[self.IDENTIFIER], position))
                    elif self.is_constant(tokens[i]):
                        position = self.get_position(self.constants_symbol_table, tokens[i])
                        self.program_internal_form.append((self.codification_table[self.CONSTANT], position))
                    else:
                        raise LexicalException(f"Unexpected token at {index}, {column}: {tokens[i]}")
