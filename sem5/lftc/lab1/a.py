...
separators = ['[', ']', '{', '}', '(', ')', ';', ' ',','] 
operators = ['+', '-', '^', '*', '/', '<', '<=', '=', '>=', '>', '>>', '<<', '==', '&&', '||', '!']
reservedWords = ['int', 'char', 'bool', 'float', 'array', 'struct', 'if', 'else', 'for', 'while', 'out', 'true', 'false']
...
class HashTable(): 
    ...
    def __found(self, key):
    	for value in self.__content:
    		if(value[0] == key):
    			return True
    		#print(value[0], key)
    	return False
    ...
    def add(self, value):
        ...
        aux = self.__found(key)

        if aux is True: 
            collisionList = self.__content[index]
            if value not in collisionList:
                l = list(collisionList)
                l.append(value)
                collisionList = l
                self.__content[index] = collisionList

            return (index, collisionList.index(value)-1)
        else: 
    ...
    for line in file:
        ...
        for token in tokenGenerator(line[0:-1], separators):  
            if token in separators + operators + reservedWords: 
                if not isEscapedSpace(token):
                    pif.add(codification[token], -1)
                    print("Separators, operators, reservedWords", token, codification[token])
               ...
               # print("len", len(token))
                ...
                print("Constants", codification['constant'], token, id)
            else:
