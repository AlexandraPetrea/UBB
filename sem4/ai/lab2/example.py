...
def fitness(individual):
    ...
    n = len(individual)
  #  f = 0;
  #  f = -20 * numpy.exp(-0.2 * sqrt(0.5 * (x**2 + y**2))) - numpy.exp(0.5 * (numpy.cos(2 * pi * x) + numpy.cos(2 * pi * y))) + 2.71 + 20
   ...
   # f = -0.0001 * (abs(np.sin(x) * np.sin(y) * np.exp(100 - np.sqrt(x**2 + y**2) / np.pi)) + 1) ** 0.1
    return f
